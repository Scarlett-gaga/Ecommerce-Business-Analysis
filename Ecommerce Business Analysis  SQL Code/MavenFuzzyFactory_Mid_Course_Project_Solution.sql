...
*/ 
-- 这是总体数据，但题目要求的是monthly trend
SELECT 
	ws.utm_campaign,
    COUNT(ws.website_session_id) AS sessions,
    COUNT(o.order_id) AS number_of_orders,
    COUNT(o.order_id)/COUNT(ws.website_session_id) AS session_to_order_conversion_rate
...
SELECT
    ...
    -- 哈哈这里有个小改动，如果把DISTINCT去掉，把website_session_id换成1就可以。
    -- 不然有DISTINCT的话，所有的1重复了
...
-- we find the first date where we start to apply lander-1 as landing page
...
-- for this step, we'll find the first pageview id for each session
